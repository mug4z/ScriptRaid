...
echo 3. Formatage du système de fichiers du RAID en ext4
mkfs -t ext4 /dev/md0

sleep 1

read -p "4. Choissiez le point de montage du RAID : " MountPoint
mkdir -p $MountPoint

sleep 1

echo 5. Montage du RAID dans le dossier $MountPoint
mount /dev/md0 $MountPoint

sleep 1

echo 6. Récupération du PATH du RAID et de son UUID
UUIDmd0=$(blkid -s UUID -o value /dev/md0)

sleep 1

echo 7. Rendre le point de montage persistant
tofstab="UUID=$UUIDmd0 $MountPoint ext4 defaults,nofail 0 2"
echo $tofstab >> /etc/fstab

sleep 1

echo 8. Rendre le raid peristant en cas de redémarrage
mdadm --verbose --detail --scan >> /etc/mdadm.confs

sleep 1

echo 9. Vérification du niveau du RAID et le nombre de disques durs actifs
